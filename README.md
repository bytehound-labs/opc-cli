...
- **Hierarchical Browsing**: Recursive tag discovery for the TUI plus bounded, one-level native browse pages with OPC DA 3.0 support and a one-time, narrowly classified DA 2.x compatibility fallback before the first successful true-root page. Full hierarchical inventory uses deterministic breadth-first traversal. Diagnostic subtree requests use the additive `OpcProvider::start_inventory_at_root` API, remain scoped to their requested canonical ItemID when DA 3.0 is available, and create a fresh independent OPC connection rather than inferring DA2 path components or parsing vendor-specific ItemID separators.
- **Inventory Telemetry**: Library consumers can pace bounded inventory operations, adjust their batch size at runtime, and receive typed slice observations plus separate startup capability observations with per-operation counts, timings, histograms, and percentiles while preserving the legacy aggregate native-operation count.
...
- **DA2 Branch Recovery**: Server-reported DA2 branch and leaf classifications are trusted for inventory, while branch navigation is deferred until expansion. Expected non-navigable branches are skipped with a completion warning; non-progressing iterators and unrelated errors remain terminal, and an already-emitted item is preserved when a later child expansion fails.
